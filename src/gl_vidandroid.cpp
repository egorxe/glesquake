...
void GL_Init (void)
{
  ...
  #ifdef USE_SDL
  createEGLWindow(scr_width, scr_height, "GLES Quake");
  #endif
  ...
  #if !NO_BLENDING && NO_MULTITEXTURE
  gl_mtexable = false;  // !! false for no multitexture with blending !!
  #else
  gl_mtexable = true;  // !! false for no multitexture with blending !!
  #endif
...
{
    if (currentFrame > 10) {    // ignore first 10 frames
...
void FrameStep()
{
  ...
  {
      printf("Frame %d\n", host_framecount);
      printf("Fastest: %5.1f ms on frame %d\n", fastestFrame.time * 1000.0, fastestFrame.frame);
      printf("Average: %5.1f ms\n", (newtime / host_framecount) * 1000.0);
      printf("Slowest: %5.1f ms on frame %d\n", slowestFrame.time * 1000.0, slowestFrame.frame);
  }
  #endif
}
...
void GL_EndRendering (void)
{
  ...
  if (host_framecount != old_frame)
  {
      old_frame = host_framecount;
      FrameStep();
  }
}
